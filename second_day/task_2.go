...
func add(n *node, c cubo) *node {
	...
	for _, a := range Axis {
		...
		case c.max[a] < n.c.min[a]:
			return &node{a: a, v: c.max[a], left: &node{c: c}, right: n}
		case c.min[a] > n.c.max[a]:
			return &node{a: a, v: c.min[a] - 1, left: n, right: &node{c: c}}
		default:
			if v := n.c.min[a]; c.min[a] < v && c.max[a] >= v {
				in, out := c, c
				in.min[a], out.max[a] = v, v-1

				return &node{a: a, v: v - 1, left: &node{c: out}, right: add(n, in)}
			}
			if v := n.c.max[a]; c.max[a] > v && c.min[a] <= v {
				in, out := c, c
				in.max[a], out.min[a] = v, v+1

				return &node{a: a, v: v, left: add(n, in), right: &node{c: out}}
			}
		}
	}
...
func main() {
	...
	fmt.Println(recount(p1))
	fmt.Println(recount(p2))
}
